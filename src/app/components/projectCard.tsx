...
export const ProjectCard = (project: Project)=>{
    ...
    const eventClickViewProject = () => {
        if(project.urlProject && project.urlProject !== ""){
            window.open(project.urlProject, '_blank');
        }
    }
    const eventClickViewVideo = () => { 
        if(project.urlVideo && project.urlVideo !== ""){
            window.open(project.urlVideo, '_blank');
        }
    }
    return <>
    <div className="group relative flex flex-col items-center justify-center gap-4 p-6 bg-white dark:bg-[#112240] rounded-lg shadow-md hover:shadow-xl transform hover:-translate-y-2 transition-all duration-300">
        ...
        <div className="w-full flex justify-end gap-2">
            {project.urlVideo && <button  className="bg-[#a30000] text-sm text-white font-semibold py-2 px-4 rounded-md "
            > <span className="">Ver Video</span></button>}
